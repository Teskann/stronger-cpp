...
using StrongBool = strong_bool<tag()>;
using WeakBool = weak_bool<tag()>;
using WeakBool2 = weak_bool<tag()>;
constexpr WeakBool True(true);
constexpr WeakBool2 False(false);

constexpr bool foo(WeakBool, WeakBool2)
{
    return true;
}
...
{
    if constexpr(supports_and<StrongBool>)
...
{
    if constexpr(supports_inclusive_or<StrongBool>)
        throw assertion_error("Bool || Bool should not be available");
    return true;
}
...
static_assert(can_call_foo<WeakBool, WeakBool2>);
static_assert(!can_call_foo<bool, bool>);
static_assert(!can_call_foo<WeakBool2, WeakBool>);
static_assert(!std::is_convertible_v<bool, StrongBool>);
static_assert(!std::is_convertible_v<StrongBool, bool>);
static_assert(!std::is_convertible_v<bool, WeakBool>);
static_assert(std::is_convertible_v<WeakBool, bool>);
