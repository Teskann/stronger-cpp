...
[![Build + Tests](https://github.com/Teskann/stronger-cpp/actions/workflows/ci.yml/badge.svg)](https://github.com/Teskann/stronger-cpp/actions/workflows/ci.yml)
...
* [Usage](#usage)
    ...
    * [Working with strong booleans](#working-with-strong-booleans)
...
double underlying_value = *price;  // Access underlying value with operator*
size_t size = alice->size();  // Access underlying methods with operator->
```
...
### Working with strong booleans

Calling functions with boolean arguments is usually confusing.
...
```C++
using EnableOptionA = stronger::strong_bool<stronger::tag()>;
using EnableOptionB = stronger::strong_bool<stronger::tag()>;

void set_options(EnableOptionA optionAEnabled, EnableOptionB optionBEnabled)
{
    // Use it as if it was a bool
    if (optionAEnabled && optionBEnabled)
        // ...
}

// Much easier to read
set_options(EnableOptionA(true), EnableOptionB(false));

// set_options(true, false);  // Does not compile
// set_options(EnableOptionB(true), EnableOptionA(false));  // Does not compile: swapped options
```

*Note: `operator&&(strong_bool, strong_bool)` and `operator||(strong_bool, strong_bool)` are not overloaded because they
would not support [short-circuit evaluation](https://en.wikipedia.org/wiki/Short-circuit_evaluation).
Instead, `strong_bool` allows implicit conversions to `bool`. However `operator!(strong_bool)` is overloaded and returns
a `bool`.*
...
alice->starts_with("A");  // Ok
alice->reset(&p); // Error, std::unique_ptr::reset is not available anymore
```
...
Use operators on the underlying type instead, or use [`strong_bool`](#working-with-strong-booleans).
